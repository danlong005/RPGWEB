        ...
        dcl-ds config likeds(RPGWEBAPP) inz;
        ...
        dcl-pi *N extpgm('RPGWEB_THD');
          ...
          exec sql select request into :data
                   from RPGWEB.RPGWEBP
                   where key = :request_id
                   and recid = 'R';

          exec sql select request into :config 
                   from RPGWEB.RPGWEBP
                   where key = :request_id
                   and recid = 'C';

          exec sql delete from RPGWEB.RPGWEBP
                   where key = :request_id;

          request = RPGWEB_parse(data);
